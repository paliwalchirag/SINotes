...
select country_name ,upper(lpad(country_name,3)) from countries;
select * from departments;
select department_name from departments where lower(department_name) like '%sales%';
select email from employees;
select split_part(email,'@',1) from employees;
select * from jobs;
select concat(upper(lpad(job_title,1)) ,substring(job_title,2,length(job_title))) from jobs;

select * from regions;
select region_name from regions where region_name ilike 'a%';

select CONCAT(lpad(first_name,15,'-'),',', rpad(last_name, 15,'-')) AS padded_name
from employees;

select * from locations;
select upper(substring(postal_code,length(postal_code)-1,length(postal_code))) from locations;

select concat(job_title ,',' ,length(job_title)) as job_title from jobs;
